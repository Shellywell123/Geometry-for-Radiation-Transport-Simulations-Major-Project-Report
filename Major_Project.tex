...
\label{motiv}
\noindent Historically it has been very hard to simulate particle radiation through matter correctly, being both expensive and time consuming. As computer technology has developed through recent years particle physicists have began building models to simulate these radiation events, employing techniques such as the Monte Carlo method. Currently many independent industries all require the ability to conduct radiation transport simulations, but work with different file formats\\
not nessescarily file formats but its methodology of storing geometries, which comes as diffenent file formats\\
\\
, which is where one of the biggest problems lie. Many software tools for simulating radiation transport are format specific and require complex file conversions. 
\\
sont talk about pyg4ometry stright away\\
the motivation of the proj is to study the eff of ttesselated solid (non stn geom). and then eaplin what tesslated solid is\\
\\
Pyg4ometry is a tool created to easily convert between these representations and allow a wider audience to utilise these radiation tracking softwares. GUImesh is a similar python package for converting STEP files into GDML format \cite{meh}.

\subsection{Project Aims}
\label{aim}
Simulating radiation transport requires a description of the target 3D formation, detailing its structure, dimensions and material properties such as density. These 3D models contain elements and volumes which can not overlap, leading to most radiation transport geometries being made by hand.
\\
these types of aims are possible in side software such as pyg4\\
the geo produced through yg4 can be simulated in softwares such as bdsim\\
and introduce geant4 (undrlying librariy of geometries,tracking,fields materials physics)\\
...
\\\\
The first part of the project focuses on the meshing of the primitive solids 
\\
what is a prim solid\\
\\
(Section \ref{prim}) within Pyg4ometry.
%next sentence doent right
...
% basically horrten it


% dont worry about dockers to much 
...
% next sentce doesnt quite make sense in para
...
\label{g4}
%g4 is not a program it is a library of c++ classes 
...
%not geom handlin its designed for creating 3d models of experiments for monte carlo simulations
 ...
 %would not mentioned pyg4 ijn new ordering yet
...
% put normal materials frst and then say in ge4 they are..
 ...
 % phylists are bdsim not g4 same with gmad files
...
\subsection{BDSIM}
\label{bdsim}
%is a c++ program based apo g4 and root which programatially builds a 3d model of a partile accelerator
% bdsim takes a simple asci input file with its own extension (GMAD), where you can controll simulation and model parmeters
...
% ref
 ...
 % G4Beamline \cite{g4beam} is a similar package, however includes the effects between particles ('collective effects') , which BDSIM does not due to better tracking and geometry in general. 
% Both BDSIM and G4Beamline alike use GEANT4 as a basis for their beamline simulations. 
...
%although be deafult ot bulds up a standard geom beamline often specaialised pieces of geom are neede to accuratley simulate interaction desired
%this can be done by supplying an external GDML file 
% basically say gmad can place objects in bea,line path or anywhere else in simulation
...
% should explainf bdsim colours before this plot apossed to later on
\begin{figure}[h!]
\centering
...
\\\\
% these tests are shown in subsequent sections
...
%check not duplicate with abve section 
...
% maybe move all this to bdsim software section
...
The meshed solid chosen to be tested within BDSIM for this project was the sphere, which is made of a minimum and maximum radius, making it a hollow solid. The reason a sphere was chosen because it is the ideal shape testing radiation in all directions, as its thickness can be controlled radially. As seen in % wrong ref
...
% for a small num of events where where a large num of secondaries results in a longer event duration
